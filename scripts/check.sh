...
# `models` (веса) и один тест с настоящим писком из колонок. `hardware` идёт:
# микрофон, экран и Диспетчер учётных данных отвечают сами, а если устройства
...
# веса и настоящий писк из колонок. Сказать об этом один раз честнее, чем
...
if ((RUN_TESTS)) && ((FULL)); then
    if [[ ${AYRIS_TEST_SPEAKERS:-} != 1 ]]; then
        echo "не проверено здесь: колонки (обрыв фразы на слух) — включить:"
