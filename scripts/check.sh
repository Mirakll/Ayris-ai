...
# * Тесты про время (маркер `slow`, 14 штук) идут вторым вызовом и
...
#
# Про маркеры: не запускаются только `models` (веса). `hardware` идёт:
# микрофон, экран и Диспетчер учётных данных отвечают сами, а на Windows полный
# прогон также включает короткий писк из настоящего устройства вывода. Если звук
# сейчас недопустим, его можно явно отключить через AYRIS_TEST_SPEAKERS=0.
# Если устройства
# нет — тест говорит «скип» и почему, а не красит прогон. `network` тоже идёт:
# сеть на этой машине есть, а в CI network не идёт нигде, чтобы падение чужого
# сайта не красило четыре джоба. То есть единственный прогон тестов, которые
# правда ходят в интернет, — вот этот.
...
# Проверка колонок раньше требовала ручного AYRIS_TEST_SPEAKERS=1 и поэтому
# обычный полный прогон и pre-push её честно объявляли непроверенной. На машине
# Windows звук является частью доступного железа, поэтому полный прогон включает
# его по умолчанию. Явный 0 остаётся аварийным выключателем для тихой среды.
if ((WINDOWS)) && [[ -z ${AYRIS_TEST_SPEAKERS+x} ]]; then
    export AYRIS_TEST_SPEAKERS=1
fi
...
pytest_start() {
    pt_common
    if ((FULL)); then
        # hardware — микрофон, колонки, экран, реальный keyring. Идут ниже,
        # своей стадией в тишине: устройство либо отвечает, либо тест сам
        # говорит «скип» и почему. До 05.09.2026 они были исключены совсем,
        # потому что отсутствующий эндпоинт вылезал сырым COMError-ом (см.
        # COM_ERRORS в utils/winapi.py) и был неотличим от настоящей поломки.
        # slow — тесты про время, там же ниже.
...
# воркеров pytest уже нет, поэтому их таймингам никто не мешает. Следом, тоже
# последовательно и по той же причине, идёт железо: микрофону, который две
# секунды слушает комнату, тишина нужна ровно так же.
if ((RUN_TESTS)) && ((FULL)); then
    pt_common
    stage slow "$PY" -m pytest "${PT[@]}" -m slow
    pt_common
    stage hardware "$PY" -m pytest "${PT[@]}" -m hardware
fi
...
# Что просит переменную в AYRIS_TEST_*, без неё тихо пропускается: настоящие
# веса. Колонки на Windows включены выше по умолчанию. Сказать об этом один раз
# честнее, чем
# показывать «12 skipped» и оставлять человека гадать, что именно не проверено.
# Сам скрипт весов не качает: он ходит на huggingface за faster-whisper при
# каждом запуске и стоит полторы минуты, а в CI эти тесты идут отдельным джобом.
if ((RUN_TESTS)) && ((FULL)); then
    if [[ ${AYRIS_TEST_SPEAKERS:-} != 1 ]]; then
        echo "не проверено здесь: колонки явно отключены — включить:"
        echo "  AYRIS_TEST_SPEAKERS=1 scripts/check.sh   # будет короткий писк"
    fi
