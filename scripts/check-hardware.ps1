...
<#
    ...
    не запускаются, потому что раннеру нечем их выполнить. Восемь из десяти
    закрыты в песочнице Claude (движки и модели туда доустановлены), а два
    закрыть там нечем принципиально:
      ...
      * колонки - того же свойства: проверяется, что звук ОБРЫВАЕТСЯ мгновенно
        по «стоп», а на null-устройстве обрывается что угодно.
