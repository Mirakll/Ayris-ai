...
Зачем отдельный скрипт. Восемь тестов проверяют то, чего заглушка проверить не
может: что Vosk действительно распознаёт фикстуру, что Piper действительно
выдаёт звук, что openWakeWord вообще загружается. Весов в репозитории нет и не
будет — Vosk это 46 МБ, — поэтому раньше эти тесты не запускались нигде, кроме
машины разработчика, и пять настоящих багов в путях к моделям всплыли только
тогда, когда я эти веса наконец скачал.
...
#: Модель CTranslate2 для faster-whisper. В каталоге её нет и быть пока не
#: может: экспорт CT2 — это каталог из четырёх файлов, а `ModelFile.target`
#: запрещает разделители пути, так что положить `config.json` в свою подпапку
#: каталогом нельзя, а плоско — значит столкнуть два разных whisper'а на общем
#: имени. Это дырка в схеме каталога, отдельная задача; здесь просто HF.
