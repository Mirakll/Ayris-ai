...
- Автодискавери импортирует все модули `actions/system/*`, а они из задач 20–29 тянут WinAPI, pycaw и WinRT. Если такой импорт стоит на верхнем уровне модуля, в Linux-песочнице упадёт сам сбор тестов, а не отдельный тест. Тяжёлые и платформенные импорты убирать внутрь функций, а реестр покрывать тестовыми действиями-пустышками, не реальными системными.
- Проверка таймаута для sync-действия должна убеждаться не только в том, что вернулся `ActionTimeout`, но и в том, что зависший поток помечен и не считается свободным: поток в пуле по `wait_for` не прерывается, и без такой проверки тест зелёный при текущем пуле.

## Готово когда
- [ ] `registry.execute()` на тестовом действии-пустышке выполняется, битые параметры дают `ActionParamsInvalid` с понятным сообщением.
