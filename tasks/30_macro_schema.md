...
## Готово когда
- [x] Оба примера из раздела 22 ТЗ загружаются в `CommandModel` и сериализуются обратно без потери данных (round-trip).
- [x] Битые параметры блока, ссылка на несуществующую переменную и цикл `CallCommand` дают понятные ошибки валидации с путём до блока.
- [x] Файл с более старым `schema_version` подхватывается через миграцию, с более новым — отклоняется с понятным сообщением.
- [x] Команда сохраняется в БД и читается обратно идентичной.
- [x] Комбинации во всех нотациях (`ctrl+alt+v`, `Ctrl + Alt + V`, запись VoiceAttack и AutoHotkey) приводятся к одному каноническому виду.
- [x] Тесты зелёные, ruff, black и mypy strict проходят.
